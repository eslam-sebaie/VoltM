...
class APIManager {
    ...
    class func getServices(country_id: Int, city_id: Int, completion: @escaping(Result<ServiceResponse, Error>) -> Void) {
        request(APIRouter.getServices(country_id, city_id)) { (response) in
            completion(response)
        }
    }
    ...
    class func getAllServcies(completion: @escaping(Result<allServicesResponse, Error>) -> Void ) {
        request(APIRouter.allServcies) { (response) in
            completion(response)
        }
    }
